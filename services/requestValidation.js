...
export const validation = function(request, rest) {
    ...
    if (!request.infMkbRequestNum) {
      errors.push(
        'Поле "Номер Заявки размещения договора управления в ГИС ЖКХ" в блоке ' +
          '"Сведения о договоре управления многоквартирным домом" обязательно для ввода и должно состоять из 9 цифр'
      )
    } else {
  ...
  if (request.typeId === 8 || request.typeId === 9) {
    if (
      ...
      errors.push(
        'Поле "Номер Заявки размещения договора управления в ГИС ЖКХ" в блоке "Дополнительные сведения" ' +
          'обязательно для ввода и должно состоять из 9 цифр'
      )
    }
  }

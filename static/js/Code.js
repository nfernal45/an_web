...
{
    console.log("include_async_code")
    ...
    fileref.setAttribute("src", "/management/js/async_code.js");
    fileref.setAttribute("id", "async_code");
...
function Common_CheckForPlugIn() {
    console.log("Common_CheckForPlugIn")
...
function SignCadesXML_NPAPI(certListBoxId, signatureType) {
    ...
    catch(err)
    {
        ...
        document.getElementById("SignatureTxtBox").innerHTML = JSON.stringify(err);
    }
}
...
function CheckForPlugIn_NPAPI() {
    ...
    function GetCSPVersion_NPAPI() {
        ...
        setStateForCSP("green", "Криптопровайдер загружен");
                ...
                if(xmlhttp.status == 200) {
                    ...
                    if (isPluginWorked) { // плагин работает, объекты создаются
                        if (VersionCompare_NPAPI(PluginBaseVersion, CurrentPluginVersion)<0) {
                            setStateForPlugin("yellow", "Плагин загружен, но есть более свежая версия");
                        }
                    }
                        ...
                        if (isPluginLoaded) { // плагин загружен
                            if (!isPluginEnabled) { // плагин загружен, но отключен
                                setStateForPlugin("red", "Плагин загружен, но отключен в настройках браузера");
                            }
                            else { // плагин загружен и включен, но объекты не создаются
                                setStateForPlugin("red", "Плагин загружен, но не удается создать объекты. Проверьте настройки браузера");
                            }
                        }
    ...
    try {
        ...
        setStateForPlugin("green", "Плагин загружен");
        setStateForCSP("yellow", "КриптоПро CSP не загружен");
...
function FillCertList_NPAPI(lstId) {
    ...
    try {
        ...
        if (!lst) {
            setStateForObjects("red", "Ошибка при перечислении объектов плагина");
            return;
        }
    }
    catch (ex) {
    ...
    setStateForObjects("green", "Перечисление объектов плагина завершено");
}
...
function ShowEdgeNotSupported() {
    setStateForPlugin("red", "К сожалению, браузер Edge не поддерживается!");
}
